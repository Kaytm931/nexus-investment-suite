---
name: gsap
description: Create advanced animations with GSAP. Use when building complex timelines, scroll-driven animations, SVG morphing, text animations, or high-performance web animations beyond CSS capabilities.
---
...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `terminalskills-skills-gsap`                                |
| Version    | 1.0.1                                     |
...
**GitHub:** [TerminalSkills/skills](https://github.com/TerminalSkills/skills) — ⭐ 5 | Forks: 1
...
GSAP (GreenSock Animation Platform) is a high-performance JavaScript animation library for building complex, production-quality web animations. It offers imperative APIs (gsap.to, gsap.from, gsap.set), timeline sequencing with precise playback controls (play, pause, reverse, seek), and staggering/easing for polished choreography. With plugins like ScrollTrigger you can create scroll-driven effects, pinned sections, scrubbed horizontal scrollers, and enter/leave toggles. GSAP also supports SVG morphing, rich text effects, and physics-based motion for natural interactions. Use it for landing pages, product showcases, interactive UI transitions, animated content, and micro-interactions where CSS cannot provide the needed control or performance. Core advantages include frame-perfect timing, broad browser support, minimal layout thrashing, and an extensible plugin ecosystem used by major brands.
...
```bash
npx -y @lobehub/market-cli skills install terminalskills-skills-gsap
```
...
npx -y @lobehub/market-cli skills install terminalskills-skills-gsap --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install terminalskills-skills-gsap --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install terminalskills-skills-gsap --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install terminalskills-skills-gsap --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install terminalskills-skills-gsap --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate terminalskills-skills-gsap --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment terminalskills-skills-gsap -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment terminalskills-skills-gsap
```
...
```bash
npx -y @lobehub/market-cli skills comments terminalskills-skills-gsap
```

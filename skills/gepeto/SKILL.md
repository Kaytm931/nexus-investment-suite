...
Set `$PINOKIO_HOME` to your Pinokio install root before following any path reference in this guide:

- macOS/Linux (default): `export PINOKIO_HOME="$HOME/pinokio"`
- Windows (default): `set PINOKIO_HOME=D:\pinokio`

All paths in this document use `$PINOKIO_HOME` as a placeholder for that root.
...
- In most cases, the `README.md` file in the `$PINOKIO_HOME` folder contains the instructions needed to install and run the app, but if not, figure out by scanning the rest of the project files.
